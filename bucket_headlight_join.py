...
def bucket_headlight_join():
    ...
    top_arc = lambda rad, bigger = False: arc(rad = rad, start_degrees = -40 - (1 if bigger else 0), end_degrees = 40 + (1 if bigger else 0))

    top_part = top_arc(outer_r + thick)
    top_part -= top_arc(outer_r - thick - 7, True)
    top_part -= top_arc(outer_r) - top_arc(outer_r - 7)
    top_part = linear_extrude(20)(top_part)
    top_part += down(base_h)(linear_extrude(base_h + thick)(
        top_arc(inner_r + thick) - top_arc(inner_r - thick - 1, True)
    ))
    ...
    screw_point = rotate(90, RIGHT_VEC)(screw_point)
    screw_point = rotate(90, FORWARD_VEC)(screw_point)
    screw_point = translate([outer_r, dash_link_gap / 2, 30 - (thick * 2)])(screw_point)
